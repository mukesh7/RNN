...
#print(tt.get_shape())
#tt1 = tf.layers.dense(tt,256,use_bias=False)
#drop = tf.nn.dropout(tt, 0.8)


#tt = tf.transpose(tt)
#tt=tf.cast(tt,tf.float32)
...
#   decoder_hidden_units, attention_states)
#
#
...
#    for batch in range(len(valin)-1):
    ...
    score_tmp = bleu_score.scoring_func('ref.txt', 'pre.txt')
    score.append(score_tmp)
    
    print('epoch: {}, train loss: {}, score : {}'.format(epoch,loss_epoch,score_tmp))
...
#    print('epoch: {}, val loss:  {},lr: {}'.format(epoch,loss_val,LEARNING_RATE))
    ...
    index_in_epoch=0
    index_in_epoch2=0   
np.savetxt("plot_bleu.csv", np.column_stack((t_loss, score)), delimiter=",")
