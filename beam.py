...
#    po=tf.cast(previous_output,tf.float32)
    ...
    def get_next_input():
#        print(previous_output.get_shape())
#        po1= tf.concat((tt,previous_output),0)
#        print("dfdfdf")
        v = tf.tanh(tf.tensordot(inputs, w_omega, axes=1) + b_omega)
        vu = tf.tensordot(v, u_omega, axes=1, name='vu')
        alphas = tf.nn.softmax(vu, name='alphas')
        tt= tf.reduce_sum(inputs * tf.expand_dims(alphas, -1), 1)
        tt1= tf.reshape(tt,(batch_size,512))
        po1 = tf.layers.dense(tt1,512,use_bias=False)
        prev = tf.add(previous_output,po1)
        ###########DROP OUT################
#        prev = tf.nn.dropout(prev,DROP)
#        output_logits = tf.add(tf.matmul(prev_drop, W), b)
        
        output_logits = tf.add(tf.matmul(prev, W), b)  #att problem
...
decoder_initial_state = tf.contrib.seq2seq.tile_batch(
    encoder_final_state, multiplier= int(args.beam_width))
...
#maximum_iterations = decoder_lengths.eval()
###########look for decoder length###################
source_sequence_length = decoder_lengths
maximum_iterations = tf.round(tf.reduce_max(source_sequence_length))

decoder_outputs, decoder_final_state,seq_lengths = tf.contrib.seq2seq.dynamic_decode(
   decoder=decoder,
